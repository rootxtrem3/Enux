...
THUMBNAIL_WIDTH="360"  # Size of thumbnails in pixels (16:9)
...
SHUFFLE_ASSET="${SHUFFLE_ASSET:-$HOME/Repos/wallpaper-selector/assets/shuffle.png}"

# Build a shuffle thumbnail. If a custom asset is missing, draw a text fallback.
if [[ -f "$SHUFFLE_ASSET" ]]; then
    magick -size "${THUMBNAIL_WIDTH}x${THUMBNAIL_HEIGHT}" xc:#1e1e2e \
        \( "$SHUFFLE_ASSET" -resize "80x80" \) \
        -gravity center -composite "$SHUFFLE_ICON"
else
    magick -size "${THUMBNAIL_WIDTH}x${THUMBNAIL_HEIGHT}" xc:#1e1e2e \
        -gravity center -pointsize 48 -fill white -annotate +0+0 "RND" \
        "$SHUFFLE_ICON"
fi
...
if [ -n "$selected" ]; then
    ...
    if [ -n "$original_path" ]; then
        # Set wallpaper via Matugen so colors stay synced
        "$HOME/.local/bin/matugen-wallpaper" "$original_path"
